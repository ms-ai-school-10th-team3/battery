...
def save_ct_record(uploaded_file, selected_line):
    now = datetime.now()
    ct_id = f"CT-{len(st.session_state.ct_history) + 1:03d}"
    file_signature = f"{uploaded_file.name}-{uploaded_file.size}"

    if st.session_state.ct_saved_upload_signature == file_signature:
        return
    ...
    if st.session_state.ct_analysis_done:
        ...
        <div class="result-box">
            <div class="reason-list">
                <b>불량 이유</b><br>
                <span class="reason-alert">△</span> 전극 정렬 이상<br>
                <span class="reason-alert">△</span> 내부 공극 이상<br>
...
with main_left:
    ...
    if analyze_clicked:
        if uploaded_file:
            ...
            save_ct_record(uploaded_file, selected_line)
