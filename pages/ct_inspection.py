...
#     confidence = 0.90 if abnormal else 0.85
...
#                 defect_reason_html += f'<span class="reason-alert">△</span> {defect} 감지 ({ratio}%)<br>'
#         else:
#             defect_reason_html = '<span style="color:#16A34A;">✓</span> 주요 결함 없음<br>'

#         risk_score = int(round(sum(float(v) for v in defect_ratios.values())))
#         risk_score = min(max(risk_score, 0), 100)
#         risk_level = "높음" if risk_score >= 5 else "낮음"
#         html(f"""
...
#             <div class="fail-text" style="font-size:26px; margin-top:8px;">{risk_level}</div>
...
#                 <b>검사 시간</b>
#                 <div>
#                     분석 소요 시간&nbsp;&nbsp; <b>{st.session_state.latest_ct_result.get("elapsed_sec", "-")}초</b><br>
...
def render_detail_data():
    ...
    if latest:
        ...
        detail_df = pd.DataFrame({
            "항목": ["Model", "Prediction", "Confidence", "Risk Score", "Defect Types", "Elapsed"],
            "값": [
